process jbrowse {
  ...
  tuple val(prefix), file(gff), file(draft), file("prokka_gff"), file(mlst), file(barrnap),
        ...
        file(phigaro), file(genomic_islands), file("methylation"), file("chr.sizes"), file(phispy_tsv)
  ...
  ./run_jbrowse.sh -p $prefix -g $draft -b $gc_bedGraph -s $gc_chrSizes -f $gff -r $barrnap -B $phigaro -P $phispy_tsv -G $genomic_islands -m methylation -S chr.sizes
  """
}
